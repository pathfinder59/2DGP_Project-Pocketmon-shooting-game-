...
class P_bullet(Bullet):
    def __init__(self,x,y):
        super().__init__(x,y)
        P_bullet.image=load_image(fileLink+'Bullet\\Bullet.png')
    def update(self):
    ...
    def draw(self,player):
        P_bullet.image.clip_draw((2 - player.Type) * 10, 0, 10, 10, self.x, self.y)
...
class E_bullet(Bullet):
    def __init__(self,x,y,angle,angle_rate,speed,speed_rate):
        ...
        self.frame=0
        self.image = load_image(fileLink+'Bullet\\Poketball.png') #frame&7 조절 하나당 크기10x10
    def update(self):
        global PI
